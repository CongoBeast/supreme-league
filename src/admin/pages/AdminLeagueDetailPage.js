...
export default function AdminLeagueDetailPage() {
  ...
  return <>
    ...
    <Card className="admin-card admin-table-card">{members.length ? <Table responsive hover className="admin-table"><thead><tr><th>Rank</th><th>Member</th><th>Joined</th><th>Eligibility</th><th>Score</th><th>Score detail</th><th>Prize</th><th>Action</th></tr></thead><tbody>{members.map((entry) => <tr key={entry.entryId}><td>{entry.rank || '—'}</td><td>{entry.user?._id ? <Link to={`/admin/users/${entry.user._id}`} className="text-decoration-none"><div className="admin-table-primary text-primary">{entry.user?.fullName || 'Unknown user'}</div><div className="admin-table-secondary">{entry.user?.email || '—'}</div></Link> : <><div className="admin-table-primary">{entry.user?.fullName || 'Unknown user'}</div><div className="admin-table-secondary">{entry.user?.email || '—'}</div></>}</td><td>{dateTime(entry.joinedAt)}</td><td><StatusBadge value={entry.eligibilityStatus} /></td><td>{entry.score || 0}</td><td className="small">{entry.scoreDetails?.captain ? `${entry.scoreDetails.captain.name} ${entry.scoreDetails.captain.points} + ${entry.scoreDetails.viceCaptain?.name || 'Vice'} ${entry.scoreDetails.viceCaptain?.points || 0}` : 'FPL GW points'}</td><td>{money(entry.prizeCents)}</td><td><div className="d-flex flex-wrap gap-2">{entry.user?._id && <Button as={Link} to={`/admin/users/${entry.user._id}`} size="sm" variant="outline-primary">View user</Button>}<Button size="sm" variant="outline-danger" disabled={saving || league.status === 'settled'} onClick={() => removeMember(entry.user?._id, entry.user?.fullName)}>Remove</Button></div></td></tr>)}</tbody></Table> : <AdminEmpty title="This league has no members" message="Use the form above to add an existing user." />}</Card>
  </>;
}
