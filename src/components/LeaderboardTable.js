  ...
  return (
    <>
                ...
                <td>
                  <div className="d-flex align-items-center gap-2">
                    {row.userId ? (
                      <button
                        type="button"
                        className="leaderboard-player-trigger"
                    ...
                    {row.userId && (
                      <button
                        type="button"
                        className="leaderboard-profile-info"
                        onClick={() => openPlayer(row)}
                        aria-label={`More information about ${row.name}`}
      ...
      <UserPublicProfileModal
        show={Boolean(selectedPlayer)}
        onHide={() => setSelectedPlayer(null)}
        userId={selectedPlayer?.userId || ''}
        initialName={selectedPlayer?.name || 'Supreme player'}
        initialPicture={selectedPlayer?.profilePicture || ''}
      />
    </>
  );
}
